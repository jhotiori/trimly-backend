...
public record ServicoCreateDTO(
        @Schema(description = "Nome do serviço, único no catálogo", example = "Corte Infantil") @NotBlank(
                message = "Nome não pode ser vazio"
        ) String nome,

        @Schema(description = "Preço cobrado, em reais", example = "35.00") @NotNull(
                message = "Valor não pode ser vazio"
        ) @Positive(message = "Valor deve ser positivo") BigDecimal valor,

        @Schema(description = "Duração do atendimento em minutos", example = "30") @NotNull(
                message = "Duração não pode ser vazia"
