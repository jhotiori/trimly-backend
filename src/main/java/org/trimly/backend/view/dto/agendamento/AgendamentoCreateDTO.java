...
public record AgendamentoCreateDTO(
        @Schema(description = "Dia do atendimento, no formato yyyy-MM-dd", example = "2027-03-15") @NotNull(
                message = "Data não pode ser nula"
        ) @FutureOrPresent(message = "Data deve estar no presente ou futuro") LocalDate data,
        ...
        @Schema(description = "Identificador do usuário atendido", example = "2") @NotNull(
                message = "Usuário não pode ser nulo"
        ) @Positive(message = "Id de Usuário deve ser positivo") Long usuarioId,

        @Schema(description = "Identificador do serviço a ser realizado", example = "1") @NotNull(
                message = "Serviço não pode ser nulo"
        ) @Positive(message = "Id de Serviço deve ser positivo") Long servicoId
) {}
